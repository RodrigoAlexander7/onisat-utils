...
# ── Serial port ──────────────────────────────────────────────────────────────
PORT     = "COM5"        # Change to your port (Linux: "/dev/ttyUSB1")
BAUDRATE = 9600
...
def receive_image():
    ...
    with serial.Serial(PORT, BAUDRATE, timeout=5.0) as ser:
        ...
        while True:
            ...
            # 1. Recuperar binarios desde Base64 (ignora espacios de padding)
            try:
                rs_data = base64.b64decode(parsed["data"])
            except Exception:
                continue

            # 2. Ejecutar decodificación de corrección de errores (FEC)
            try:
                decoded_data, _, _ = rs.decode(rs_data)
            except reedsolo.ReedSolomonError:
                print(f"  [X] Paquete con errores incorregibles desde {parsed['src_addr'].hex().upper()} (Ignorado)")
                continue

            result = parse_chunk(decoded_data)
            if result is None:
